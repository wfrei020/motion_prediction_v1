...
from tensorflow import reshape, newaxis, ones, shape, tile, int64, concat, constant, int32
...
class RunModel():
    ...
    def validate_save_samples(self):
        ...
        for set in tqdm_datasests:
            dataset = Util.get_test_dataset_segment(config.BASE_VALIDATION_PATH+ str(set).zfill(5) + "-of-00150")
        ...
        for set in tqdm_datasests:
            ...
            for batch in tqdm_dataset:
                ...
                for mini_step, mini_batch in enumerate(batch['input_states']):
                    ...
                    # print image
                    img = ig.visualize_scenario(mini_batch[:,:10,:2],
                                                            mini_batch[:,10:,:2],
                                                            pred_trajectory,
                                                            batch['gt_future_states'][mini_step][:,11:],
                                                            road_xyz[mini_step],road_type[mini_step],
                                                            batch['tracks_to_predict'][mini_step],
                                                            1000
                                                            )
                    

                    plt.imsave(f'image_results/result_{mini_step}.jpeg', img)
                quit()
